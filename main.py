...
async def crear_equipo(equipo: Equipo, conn = Depends(connect_to_postgres)):
    equipo_id = await conn.fetchval("INSERT INTO equipos (nombre, deporte_id, logo_url) VALUES ($1, $2, $3) RETURNING id",
                                    equipo.nombre, equipo.deporte_id, equipo.logo_url)
    return {"id": equipo_id, "nombre": equipo.nombre, "deporte_id": equipo.deporte_id, "logo_url": equipo.logo_url}

@app.get("/equipos/", response_model=List[Equipo])
async def listar_equipos(conn = Depends(connect_to_postgres)):
    equipos = await conn.fetch("SELECT id, nombre, deporte_id, logo_url FROM equipos")
    return [{"id": equipo['id'], "nombre": equipo['nombre'], "deporte_id": equipo['deporte_id'], "logo_url": equipo['logo_url']} for equipo in equipos]
...
async def actualizar_equipo(equipo_id: int, equipo: Equipo, conn = Depends(connect_to_postgres)):
    await conn.execute("UPDATE equipos SET nombre = $1, deporte_id = $2, logo_url = $3 WHERE id = $4",
                       equipo.nombre, equipo.deporte_id, equipo.logo_url, equipo_id)
    return {"id": equipo_id, "nombre": equipo.nombre, "deporte_id": equipo.deporte_id, "logo_url": equipo.logo_url}
...
async def eliminar_equipo(equipo_id: int, conn = Depends(connect_to_postgres)):
    equipo = await conn.fetchrow("SELECT nombre, deporte_id, logo_url FROM equipos WHERE id = $1", equipo_id)
    if not equipo:
